    ...
    <div class="register_form">
      ...
      <flexbox :gutter="0" justify="center">
        <flexbox-item :span="8.8" class="btn_box">
          <x-button class="register_btn" @click.native="submitForm">注册</x-button>
          <div class="agree_box">
            <check-icon :value.sync="isagree" type="plain">已阅读并同意</check-icon>
            <a href="/xieyi">《接单宝平台服务协议》</a>
          </div>
        </flexbox-item>
      </flexbox>
    </div>
      ...
      if (this.isagree) {
        ...
        if (arr.length === 0) {
          ...
          if (this.formdata.invitephone !== '') {
            const res2 = reg.test(this.formdata.invitephone)
            if (!res2) {
              this.myalert('推荐人手机号格式有误')
              this.isSubmit = false
              return false
            } else {
              this.isSubmit = true
