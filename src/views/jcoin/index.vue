<template>
<div style="background:#f6f5f4;padding-bottom:16px;">
<scroll ref="scroll" class="wrapper"
  ...
  <div class="integral_box">
    <div class="residue_integral">
      <p class="integral_title">J币余额</p>
      <p class="integral_num">{{num}}</p>
    </div>

    <div class="integral_detail" v-show="false">
      <h1 class="detail_title"><svg-icon icon-class="jifen"></svg-icon>我的J币</h1>
    ...
    <div class="nodetails">
      <img src="../../assets/nodetail.png" alt="暂无明细">
      <p>暂无明细</p>
    </div>
  </div>
</scroll>
<x-button class="btn">前往充值</x-button>
</div>
</template>
...
export default {
  ...
  mounted () {
    ...
    if (this.num === 0) {
      this.$nextTick(() => {
        // console.log(this.$refs.scroll.style)
        // this.$refs.scroll.style.minHeight = 'calc(100 vh - 246px)'
      })
    }
  }
}
